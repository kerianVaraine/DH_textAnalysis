...
# Using window size, count weighted frequency, make fcm
# window size is based off of : ceiling(nrow(text) / no_of_sections) + 1 :: used in character frequency calcs.
...
BMN.weightedFCM <- as.tokens(BMN_text, remove_punct = TRUE) %>%  
  tokens_keep(pattern = character_vector) %>%
  fcm(context = "window", count = "weight", window = BMN_text.window)
...
# AMN weighted fcm
AMN_text.window <- ceiling(nrow(AMN_text) / 4 / 100)

AMN.weightedFCM <- as.tokens(AMN_text, remove_punct = TRUE) %>%  
  tokens_keep(pattern = character_vector) %>%
  fcm(context = "window", count = "weight", window = AMN_text.window)
